...
export const interviewHrColumns: ColumnDef<TApplication>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const hrInterview = row.original.interviews.find(
        (intv) => intv.type === "HR"
      );
      return hrInterview?.schedule
        ? new Date(hrInterview.schedule).toLocaleString("id-ID", {
            dateStyle: "full",
            timeStyle: "short",
          })
        : "-";
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return hrInterview?.confirm_deadline
        ? new Date(hrInterview.confirm_deadline).toLocaleString("id-ID", {
            dateStyle: "full",
            timeStyle: "short",
          })
        : "-";
  ...
  },
  {
    ...
    header: "Metode",
    cell: ({ row }) =>
      row.original.interviews.find((intv) => intv.type === "HR")?.method ?? "-",
  },
  {
    id: "interview_detail",
    header: "Detail Interview",
    cell: ({ row }) => {
      const intv = row.original.interviews.find((intv) => intv.type === "HR");
      if (!intv) return "-";
